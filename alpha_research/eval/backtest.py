"""
Event-driven, cost-aware backtest of a predictive signal.

Ported from ``jepa-trader`` (``jepa_trader/eval/backtest.py``); behavior is
preserved verbatim. The engine is asset-agnostic: it operates purely on aligned
``signal`` / ``returns`` / ``spread`` / ``segment`` arrays and a horizon, so it
applies equally to equities microstructure, crypto bars, FX, etc.

Realism:
  * Non-overlapping holds: within each segment, decisions are spaced
    >= horizon apart, so a position is closed before the next is opened. A
    "segment" is any contiguous block that must not be crossed by a hold
    window (e.g. a (symbol, day) block); positions never straddle a boundary.
...
All return / spread / fee quantities are in basis points.
"""
from __future__ import annotations
import numpy as np

# Reference annualization horizon for sub-minute / event-driven signals on a
# fixed time grid: regular-trading-hours seconds per year (~5.9e6 s). For
# lower-frequency signals, prefer a bars-per-year Sharpe annualization upstream.
...
def _nonoverlap_idx(seg_id: np.ndarray, h: int, stride: int) -> np.ndarray:
    ...
    This is the backtest's purge mechanism: a hold of length ``h`` (in grid
    steps of size ``stride``) is never allowed to overlap the next decision or
    to straddle a segment edge, so realized P&L of consecutive trades is
    non-overlapping and the resulting return series is i.i.d.-friendly for a
    t-stat / Sharpe.
    """
...
def backtest_signal(signal, ret_bps, spread_bps, seg_id, h, stride,
    ...
    """Backtest one signal at one horizon across a selectivity sweep.

    Parameters
    ----------
    signal : array, the predictive signal (sign = direction, |.| = conviction).
    ret_bps : array, realized mid-to-mid forward return over the horizon, in bps
        (NaN where unavailable / out of bounds).
    spread_bps : array, contemporaneous bid-ask spread in bps (round-trip cost
        base; a buy crosses half the spread, a sell the other half).
    seg_id : array, segment identifier per row (e.g. hashed (symbol, day)).
        Holds never cross a change in ``seg_id``.
    h : int, hold horizon in grid steps.
    stride : int, dataset stride between consecutive rows in grid steps.
    fee_bps : per-side fee in bps (round-trip fee = 2 * fee_bps).
    trade_fracs : tuple, top-|signal| fractions to trade (1.0 = trade everything).
    grid_ms : int, milliseconds per grid step (only used to annualize the Sharpe
        estimate for high-frequency signals).

    Returns a dict keyed by trade_frac, each with n_trades, mean gross/net bps,
    mean cost, hit-rate, total net bps and an annualized Sharpe estimate.
    """
...
def run_from_probe(npz_path: str, methods=None, horizons=None, eval_stride: int = 32,
    ...
    """Run the backtest over every (method, horizon) stored in a probe ``.npz``.

    Expects arrays: ``y`` (T, H), ``y_mask`` (T, H), ``last_spread_bps`` (T,),
    ``seg_id`` (T,), ``horizons`` (H,), and one ``pred__{method}__h{h}`` per
    method/horizon. Mirrors the jepa-trader probe export format.
    """
    z = np.load(npz_path)
...
def print_backtest(bt: dict, trade_frac: float = 0.1) -> None:
    """Pretty-print the net-bps and annualized-Sharpe tables of a backtest dict."""
